...
export default {
  ...
  data () {
    return {
      ...
      verticalLine: [],
      transverseLine: [],
      dashboardUuid: this.$route.params.id
    }
  },
  computed: {
    ...mapState({
  ...
  methods: {
    addChartTemplate (chartList) {

    },
    setDashboardStyle () {
      this.daboardStyle = {}
      let daboardStyle = this.daboardStyle
      if (this.dashboard.config.type == 1) {
        daboardStyle['background'] = this.dashboard.config.background
      } else {
        daboardStyle['background-image'] = `url('${this.dashboard.config.background}')`
        daboardStyle['background-repeat'] = "no-repeat"
        daboardStyle['background-size'] = "100% 100%"
      }
    },
    async initDashboard () {
      let res = await getOne({ uuid: this.dashboardUuid })
      if (res && res.data) {
        let curDashboard = { ...res.data }
        if (res.data.config) {
          curDashboard.config = JSON.parse(res.data.config)
        }
